...
$(document).ready( function() {
  ...
  if(loginId == null || loginId == "")
      ...
      $("#user_div").html("<h1>Welcome "+loginId+"!! </h1>");
});
...
function resultList(name, results) {
  ...
  this.render = function() {
    ...
    showMore.href = "javascript:appendSearchLibrary('"+ $("#query-input-id").val()+"');";
...
$(document).ready( function() {
  ...
  $("#query-input-id").keyup(function (e) {
    var query = e.target.value;
    var results = $("#results_div");
    if(query != ""){
    ...
    $("#query-limit").val(6);
    searchLibrary(query, 3);
  });
  $("#query-input-id").focus(function (e) {
...
function searchLibrary(query, limit) {
    ...
    makeNetworkRequest(query, limit).then(function(response) {
...
function appendSearchLibrary(query) {
  let limit = $("#query-limit").val();
  $("#query-limit").val(parseInt(limit)+3);
  makeNetworkRequest(query, limit).then(function(response) {
    searchLibrary.cache[encodeURIComponent(query)] = response;
    writeResults(response);
  }, function(e) {
 ...
 * commad in cmd
 * 
 * curl -X POST "https://accounts.spotify.com/api/token" -H "Content-Type: application/x-www-form-urlencoded" -d "grant_type=client_credentials&client_id=b51d352e2fa64c8d8caab68d8b340dbd&client_secret=8cc83e8e3c3e4d2d859efe5a9c8f9d21"
 */
function makeNetworkRequest(query, limit) {
  return new Promise(function(resolve, reject) {
    // var key = "BQBHVHlpFcMFo7A3fO5d0YNhaa8DrsBjXSTRoiPzB--rVFR3_fjFKYJ0GakwgLKdNAwOuVajIPRPvjGae-6Rgj-1Qot0EZs7DauyG0kb3Ik8bfWbsUM";
    var key = "BQCD1zd-MSrXrshkv9zY7Z_kepXUJrkatrXw1aolJPOGGZgJTNz5S74rvV_MlaHG1WzpOUt4kpO6XxLm4_qLTbK6kSpM_HdpEW2QUSj04NpGeamEOy0";

    var API_URL = "https://api.spotify.com/v1/search?limit="+limit+"&type=artist,track,album,playlist&q=" + encodeURI(query);
